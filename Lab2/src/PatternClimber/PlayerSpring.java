...
public class PlayerSpring implements MouseMotionListener, KeyListener {
	...
	public void draw(Graphics2D of) {
		BufferedImage head = null, body = null, LRad = null, RRad = null, LHum = null,
				RHum = null, RFem = null, LFem = null, LTib = null, RTib = null;
		...
		/**
		*
		*Description: visually improving the climber using image pattern
		*
		*@author Lu Chen
		*/
		...
		try
		{
			head = ImageIO.read(new File("image/head.jpg"));
			body = ImageIO.read(new File("image/body.jpg"));
			LRad = ImageIO.read(new File("image/lRad.jpg"));
			RRad = ImageIO.read(new File("image/rRad.jpg"));
			LHum = ImageIO.read(new File("image/lHum.jpg"));
			RHum = ImageIO.read(new File("image/rHum.jpg"));
			LFem = ImageIO.read(new File("image/lFem.jpg"));
			RFem = ImageIO.read(new File("image/rFem.jpg"));
			LTib = ImageIO.read(new File("image/lTib.jpg"));
			RTib = ImageIO.read(new File("image/rTib.jpg"));
		...
		trans.translate(n.getX() + 0.155, n.getY() + 0.35);
		trans.rotate(Math.toRadians(180));
		trans.scale(0.0019, 0.0019);
		of.drawImage(head, trans, null);
...
//		}
		...
		AffineTransform transBody = new AffineTransform();
		transBody.translate(n.getX() + 0.2, n.getY());
		transBody.rotate(Math.toRadians(180));
		transBody.scale(0.0015, 0.0015);
		of.drawImage(body, transBody, null);

		int d = 190;
		AffineTransform transLRad = new AffineTransform();
		transLRad.concatenate(new AffineTransform((le.getX() - 0.05 - lh.getX()) / d, (le.getY() - lh.getY()) / d,
				(lh.getY() - le.getY()) / d, (le.getX() - 0.05 - lh.getX()) / d, lh.getX() - 0.05, lh.getY()));
		of.drawImage(LRad, transLRad, null);

		int w = 300;
		AffineTransform transRRad = new AffineTransform();
		transRRad.concatenate(new AffineTransform((re.getX()-0.15-rh.getX())/w, (re.getY()-rh.getY())/w,
				(rh.getY()-re.getY())/w,(re.getX()-0.15-rh.getX())/w, rh.getX(), rh.getY()));
		of.drawImage(RRad, transRRad, null);

		int wlhum = 150;
		AffineTransform transLHum = new AffineTransform();
		transLHum.concatenate(new AffineTransform((ls.getX() - le.getX()) / wlhum, (ls.getY() - le.getY() - 0.10) / wlhum,
				(le.getY() - 0.10 - ls.getY()) / wlhum, (ls.getX() - le.getX()) / wlhum, le.getX(), le.getY() - 0.10));
		of.drawImage(LHum, transLHum, null);

		int wrhum = 210;
		AffineTransform transRHum = new AffineTransform();
		transRHum.concatenate(new AffineTransform((rs.getX() - re.getX()) / wrhum, (rs.getY() - re.getY()) / wrhum,
				(re.getY() - rs.getY()) / wrhum, (rs.getX() - re.getX()) / wrhum, re.getX(), re.getY()));
		of.drawImage(RHum, transRHum, null);

		int wlFem = 340;
		AffineTransform transLFem = new AffineTransform();
		transLFem.concatenate(new AffineTransform((ln.getX() - p.getX() - 0.1) / wlFem, (ln.getY() - p.getY()) / wlFem,
				(p.getY() - ln.getY()) / wlFem, (ln.getX() - p.getX() - 0.1) / wlFem, p.getX() - 0.1, p.getY()));
		of.drawImage(LFem, transLFem, null);

		int wrFem = 320;
		AffineTransform transRFem = new AffineTransform();
		transRFem.concatenate(new AffineTransform((rn.getX() - p.getX()) / wrFem, (rn.getY() - p.getY() - 0.03) / wrFem,
				(p.getY()- 0.03 - rn.getY()) / wrFem, (rn.getX() - p.getX()) / wrFem, p.getX(), p.getY()- 0.03));
		of.drawImage(RFem, transRFem, null);

		int wlTib = 320;
		AffineTransform transLTib = new AffineTransform();
		transLTib.concatenate(new AffineTransform((lf.getX()+0.10 - ln.getX() - 0.20) / wlTib, (lf.getY() - ln.getY()+0.02) / wlTib,
				(ln.getY()+0.02 - lf.getY()) / wlTib, (lf.getX() +0.10 - ln.getX()- 0.20) / wlTib, ln.getX()- 0.20, ln.getY()+0.02));
		of.drawImage(LTib, transLTib, null);

		int wrTib = 320;
		AffineTransform transRTib = new AffineTransform();
		transRTib.concatenate(new AffineTransform((rf.getX() - rn.getX()) / wrTib, (rf.getY() - rn.getY()) / wrTib,
				(rn.getY() - rf.getY()) / wrTib, (rf.getX() - rn.getX()) / wrTib, rn.getX(), rn.getY()));
		of.drawImage(RTib, transRTib, null);
