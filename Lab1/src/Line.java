	...
	public static void main(String[] args) throws InterruptedException {
		...
		Line line = new Line() {
			void line(BufferedImage buf, int x0, int y0, int xn, int yn, int rgb) {
                ...
                /**
                *
                *Description: Using DDA to draw lines
                *
                *@author Lu Chen
                */
...
//                }
                ...
                /**
                *
                *Description: Bresenham approach               *
                *@author Lu Chen
                */
                bresenham_line(x0, y0, xn, yn, buf,rgb);
            }
		};
    ...
    public void bresenham_line(int x1, int y1, int x2, int y2, BufferedImage buf, int rgb) {
		boolean swap;
        // if point x1, y1 is on the right side of point x2, y2, change them
        if ((x1 - x2) > 0) {bresenham_line(x2, y2, x1, y1, buf, rgb); return;}
        ...
        if (Math.abs(y2 - y1) > Math.abs(x2 - x1)) {
            ...
            bresteepline(y1, x1, y2, x2, buf, rgb); return;
        }
    ...
    public void bresteepline(int x3, int y3, int x4, int y4, BufferedImage buf, int rgb) {
        // if point x3, y3 is on the right side of point x4, y4, change them
        if ((x3 - x4) > 0) {bresteepline(x4, y4, x3, y3, buf, rgb); return;}

        int x = x3, y = y3, sum = x4 - x3,  Dx = 2 * (x4 - x3), Dy = Math.abs(2 * (y4 - y3));
        int prirastokDy = ((y4 - y3) > 0) ? 1 : -1;

        for (int i = 0; i <= x4 -x3; i++) {
            buf.setRGB(Math.round(y), Math.round(x), rgb);
            x++;
            sum -= Dy;
            if (sum < 0) {y = y + prirastokDy; sum += Dx;}
        }
